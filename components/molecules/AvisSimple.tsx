...
const AvisSimples: React.FC = () => {
  ...
  const comentario1 =
    "Adoro os vinhos naturais da Minca! Uma explosão de sabores autênticos.";
  const comentario2 =
    "A Cantina Mincarone é uma descoberta incrível. Os vinhos são puros e cheios de personalidade.";
  const comentario3 =
    "Experimentei o Pinot Noir da Minca e fiquei impressionado. Um vinho natural de excelência!";
    ...
    <div className="flex flex-col max-w-60 bg-secondary p-4 rounded-lg shadow-md">
      <div className="flex flex-col items-center mt-12 mb-2 w-full">
        <div className="w-20 h-auto bg-primary rounded-full mb-3 flex items-center justify-center">
          <Image
            alt={`icone d'avatar`}
            src={`/avatars/${avatars[Math.floor(Math.random() * 3)]}`}
            width={200}
            height={200}
          ...
          <div className="flex">
            {[...Array(5)].map((_, i) => (
              <Star
                ...
                size={16}
                className={
                  i < avaliacao ? "text-primary fill-primary" : "text-primary"
                }
              />
            ))}
          </div>
      ...
      <p className="text-primary flex-1 flex flex-col justify-center text-sm text-center">
        &#34;{comentarios[Math.floor(Math.random() * 3)]}&#34;
      </p>
    </div>
  );
};
